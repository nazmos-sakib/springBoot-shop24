...
public class ShopController {
	...
	public ModelAndView addBookToCart(@RequestParam(value = "action") String action,
		...
		mv.setViewName("redirect:/bookDetails/" + action);
		try {
			Book book = new ShopService().findBook(action);
			ShoppingCart shoppingCart = (ShoppingCart)session.getAttribute("shoppingCart");
			if (shoppingCart == null) {
				shoppingCart = new ShoppingCart();
			...
			shoppingCart.addItem(book, 1);
			redirectAttributes.addFlashAttribute("successMessage", "Book added to shopping cart");
			return mv;
		} catch (Exception e) {
			redirectAttributes.addFlashAttribute("errorMessage", "Unknown error occurred");
			return mv;
		}
